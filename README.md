...
Destination identification searches the release title separately from bonus files and soundtrack names. Weak existing-series matches are excluded, and a single search candidate is not automatically accepted when model identification fails.
...
- AI-assisted title discovery from actors, plot, setting, era, or other remembered clues, with selectable Chinese and English search aliases.
- Multi-keyword search that queries each selected title separately, then merges, deduplicates, and ranks the combined result set.
...
- Move software and other non-media downloads outside Jellyfin into the configured software directory.
